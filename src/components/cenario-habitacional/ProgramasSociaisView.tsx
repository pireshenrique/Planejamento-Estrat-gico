...
import {
  PROGRAMAS_SOCIAIS_TOPICS,
  PROGRAMAS_SOCIAIS_BY_ID,
  type ProgramasSociaisTopicId
} from '../../data/cenario-habitacional/programasSociais';

interface ProgramasSociaisViewProps {
  setActivePage?: (page: string) => void;
}

export function ProgramasSociaisView({ setActivePage }: ProgramasSociaisViewProps) {
  const [selectedTopic, setSelectedTopic] = useState<ProgramasSociaisTopicId>('mcmv');
  const currentData = PROGRAMAS_SOCIAIS_BY_ID[selectedTopic];
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 sm:gap-4">
          <div>
            ...
            <p className="text-sm sm:text-[16px] text-slate-600 dark:text-slate-400">
              Acompanhamento de políticas habitacionais federais, subsídios e linhas de financiamento de interesse social.
            </p>
          </div>
        </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 sm:gap-3 p-1.5 sm:p-2 bg-indigo-50/30 dark:bg-slate-800/40 border border-indigo-100/60 dark:border-slate-700/60 rounded-2xl w-full shadow-inner">
            {PROGRAMAS_SOCIAIS_TOPICS.map((topic) => {
              ...
              return (
                <button
                  key={topic.id}
                  id={`btn-programa-${topic.id}`}
                  type="button"
                  onClick={() => setSelectedTopic(topic.id)}
                  className={`w-full px-3 sm:px-4 py-2.5 sm:py-3.5 rounded-xl sm:rounded-[14px] text-xs sm:text-[14px] font-bold transition-all duration-200 ease-out whitespace-nowrap text-center cursor-pointer border flex items-center justify-center gap-2 sm:gap-2.5 select-none active:scale-[0.98] ${
                    isSelected
                ...
                >
                  <div className="flex items-center justify-center shrink-0">
                    <Icon className={`w-5 h-5 sm:w-5 sm:h-5 ${isSelected ? 'text-white' : 'text-indigo-600 dark:text-indigo-400'}`} />
                  </div>

                  <span className="tracking-tight truncate max-w-full">{topic.label}</span>
                </button>
              );
            })}
          </div>
