...
interface TransformacaoVarejoViewProps {
  setActivePage?: (page: string) => void;
  hideHeader?: boolean;
}

export function TransformacaoVarejoView({ setActivePage, hideHeader }: TransformacaoVarejoViewProps) {
  return (
    <div className="w-full flex flex-col gap-4 sm:gap-6 md:gap-8 font-sans text-slate-800 dark:text-slate-200 animate-in fade-in duration-300">
      ...
      {/* CABEÇALHO (Ocultado quando embutido em Varejo e Canais) */}
      {!hideHeader ? (
        <div className="flex flex-col gap-3.5 sm:gap-6">
          <div className="flex items-center gap-1.5 sm:gap-2 text-xs sm:text-sm md:text-[15px] font-medium text-slate-500 dark:text-slate-400 overflow-x-auto whitespace-nowrap pb-1 max-w-full [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
            ...
            <ChevronRight className="w-3.5 h-3.5 shrink-0" />
            <span className="text-[#0c162c] dark:text-white font-bold shrink-0">Varejo em Transformação</span>
          </div>
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 sm:gap-4">
            <div>
              ...
              <p className="text-sm sm:text-[16px] text-slate-600 dark:text-slate-400">
                Mudanças no papel da loja física, novas estratégias, IA aplicada ao varejo e novos formatos comerciais.
              </p>
            </div>
          </div>
        </div>
      ...
      <div className="bg-slate-50/50 dark:bg-slate-900/30 rounded-3xl border border-dashed border-slate-200 dark:border-slate-800 p-12 text-center flex flex-col items-center justify-center min-h-[300px]">
        <div className="w-16 h-16 bg-amber-50 dark:bg-amber-900/20 text-amber-600 dark:text-amber-400 rounded-2xl flex items-center justify-center mb-4">
          <Store className="w-8 h-8" />
        </div>
        <h3 className="text-xl font-bold text-slate-900 dark:text-white mb-2">Varejo em Transformação</h3>
        <p className="text-slate-500 dark:text-slate-400 max-w-md">
          Esta página será utilizada futuramente para: mudanças no papel da loja física, novas estratégias, IA aplicada ao varejo e novos formatos comerciais. Em estruturação.
        </p>
      </div>
    </div>
  );
