...
import { Lightbulb, Cpu, ChevronRight } from 'lucide-react';
import { InternalDualNav, DualNavItem } from './InternalDualNav';
import { TendenciasProdutoView } from './TendenciasProdutoView';
import { CasaConectadaView } from './CasaConectadaView';

interface ProdutoInovacaoViewProps {
  setActivePage?: (page: string) => void;
  initialTab?: 'produto' | 'casaconectada';
}

const NAV_ITEMS: [DualNavItem, DualNavItem] = [
  {
    id: 'produto',
  ...
  {
    id: 'casaconectada',
    label: 'Casa Conectada',
    icon: Cpu
  }
];

export function ProdutoInovacaoView({ setActivePage, initialTab = 'produto' }: ProdutoInovacaoViewProps) {
  const [subTab, setSubTab] = useState<'produto' | 'casaconectada'>(initialTab);
    ...
    <div className="w-full flex flex-col gap-4 sm:gap-6 md:gap-8 font-sans text-slate-800 dark:text-slate-200 animate-in fade-in duration-300">
      ...
      <div className="flex flex-col gap-3.5 sm:gap-6">
        <div className="flex items-center gap-1.5 sm:gap-2 text-xs sm:text-sm md:text-[15px] font-medium text-slate-500 dark:text-slate-400 overflow-x-auto whitespace-nowrap pb-1 max-w-full [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
          ...
          <ChevronRight className="w-3.5 h-3.5 shrink-0" />
          <span className="text-[#0c162c] dark:text-white font-bold shrink-0">Produto e Inovação</span>
        </div>
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 sm:gap-4">
          <div>
            ...
            <p className="text-sm sm:text-[16px] text-slate-600 dark:text-slate-400">
              Evolução das propostas de valor, atributos de produto, conectividade e novas soluções para o ambiente doméstico.
            </p>
          </div>
        </div>
      </div>
      ...
      <InternalDualNav 
        items={NAV_ITEMS}
        activeId={subTab}
        onChange={(id) => setSubTab(id as 'produto' | 'casaconectada')}
      />

      {/* CONTEÚDO DA PÁGINA SELECIONADA */}
      {subTab === 'produto' ? (
        <TendenciasProdutoView setActivePage={setActivePage} hideHeader={true} />
      ) : (
        <CasaConectadaView setActivePage={setActivePage} hideHeader={true} />
      )}
