...
import {
  NORTH_AMERICA_TOPICS,
  NORTH_AMERICA_BY_ID,
  type NorthAmericaTopicId
} from '../../data/geopolitica/americaDoNorte';
import { RegionalHeader } from './RegionalNavigation';

interface AmericaDoNorteViewProps {
  setActivePage?: (page: string) => void;
}

export function AmericaDoNorteView({ setActivePage }: AmericaDoNorteViewProps) {
  const [selectedTopic, setSelectedTopic] = useState<NorthAmericaTopicId>('estados-unidos');
  const currentData = NORTH_AMERICA_BY_ID[selectedTopic];
    ...
    <div className="w-full flex flex-col gap-4 sm:gap-6 md:gap-8 font-sans text-slate-800 dark:text-slate-200">
      ...
      <div className="flex flex-col gap-3.5 sm:gap-6">
        <RegionalHeader
          ...
          subtitle="Acompanhamento econômico, comercial e industrial nos mercados da América do Norte: Estados Unidos e México."
          activeRegion="América do Norte"
          setActivePage={setActivePage}
        />

        {/* BARRA DE NAVEGAÇÃO DOS PAÍSES (ESTADOS UNIDOS E MÉXICO) */}
        <div className="w-full">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 sm:gap-3 p-1.5 sm:p-2 bg-indigo-50/30 dark:bg-slate-800/40 border border-indigo-100/60 dark:border-slate-700/60 rounded-2xl w-full shadow-inner">
            {NORTH_AMERICA_TOPICS.map((topic) => {
