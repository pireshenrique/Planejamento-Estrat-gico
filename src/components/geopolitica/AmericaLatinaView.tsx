...
import {
  ...
  LATAM_BY_ID,
  type LatamTopicId
} from '../../data/geopolitica/americaLatina';
import { RegionalHeader } from './RegionalNavigation';

interface AmericaLatinaViewProps {
  setActivePage?: (page: string) => void;
}

export function AmericaLatinaView({ setActivePage }: AmericaLatinaViewProps) {
  const [selectedTopic, setSelectedTopic] = useState<LatamTopicId>('colombia');
  const currentData = LATAM_BY_ID[selectedTopic];
    ...
    <div className="w-full flex flex-col gap-4 sm:gap-6 md:gap-8 font-sans text-slate-800 dark:text-slate-200">
      ...
      <div className="flex flex-col gap-3.5 sm:gap-6">
        <RegionalHeader
          ...
          subtitle="Acompanhamento econômico, comercial e industrial nos mercados prioritários da região: Colômbia, Chile, Costa Rica, Peru, Paraguai, Argentina, Bolívia e Equador."
          activeRegion="América Latina"
          setActivePage={setActivePage}
        />

        {/* BARRA DE NAVEGAÇÃO DOS PAÍSES (8 PAÍSES: COLÔMBIA, CHILE, COSTA RICA, PERU, PARAGUAI, ARGENTINA, BOLÍVIA, EQUADOR) */}
        <div className="w-full">
          <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-8 gap-1.5 sm:gap-2.5 p-1.5 sm:p-2 bg-indigo-50/30 dark:bg-slate-800/40 border border-indigo-100/60 dark:border-slate-700/60 rounded-2xl w-full shadow-inner">
            {LATAM_TOPICS.map((topic) => {
              const isSelected = selectedTopic === topic.id;

              return (
                <button
                  key={topic.id}
                  id={`btn-latam-${topic.id}`}
                  type="button"
                  ...
                  <div className="flex items-center justify-center shrink-0">
                    {topic.flags.map((flag) => (
                      <img
                        ...
                        alt={flag.name}
                        title={flag.name}
                        className={`w-[32px] h-[22px] object-cover rounded-[4px] shadow-sm border ${
                          isSelected
            ...
            <div className="flex items-center gap-2 sm:gap-2.5">
              ...
              <div>
                ...
                <p className="text-[11px] sm:text-xs text-slate-500 dark:text-slate-400 mt-0.5 sm:mt-1">
                  {currentData.statusSubtitle || 'Principais desdobramentos e pontos de atenção para a economia regional'}
                </p>
              </div>
            </div>
