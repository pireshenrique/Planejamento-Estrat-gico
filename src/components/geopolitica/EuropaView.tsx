...
import { EUROPA_DATA } from '../../data/geopolitica/europa';
import { RegionalHeader } from './RegionalNavigation';

interface EuropaViewProps {
  setActivePage?: (page: string) => void;
}

export function EuropaView({ setActivePage }: EuropaViewProps) {
  const currentData = EUROPA_DATA;
  ...
  return (
    <div className="w-full flex flex-col gap-4 sm:gap-6 md:gap-8 font-sans text-slate-800 dark:text-slate-200">
      ...
      <div className="flex flex-col gap-3.5 sm:gap-6">
        <RegionalHeader
          title="Europa"
          subtitle="Acompanhamento econômico, comercial e industrial nos mercados da União Europeia e Zona do Euro."
          activeRegion="Europa"
